...
{
    public $messageClass = "wilson\mailerqueue\Message";
    ...
    public function process()
    {
        ...
        if ($redis->select($this->db) && $messages = $redis->lrange($this->key, 0, -1)) {//取出redis从头到尾
            ...
            foreach ($messages as $message) {
                ...
                if ($messageObj->send()) {
                    $redis->lrem($this->key, -1, json_encode($message));//发送后删除
                }
            }
        }
        return true;
